...
import com.yakgwa.kullow.ar.SampleApplication.SampleAppRenderer;
import com.yakgwa.kullow.ar.SampleApplication.SampleAppRendererControl;
import com.yakgwa.kullow.ar.SampleApplication.SampleApplicationSession;
import com.yakgwa.kullow.ar.SampleApplication.SampleRendererBase;
import com.yakgwa.kullow.ar.SampleApplication.utils.CubeShaders;
...
import com.yakgwa.kullow.ar.SampleApplication.utils.SampleApplication3DModel;
import com.yakgwa.kullow.ar.SampleApplication.utils.SampleMath;
import com.yakgwa.kullow.ar.SampleApplication.utils.SampleUtils;
import com.yakgwa.kullow.ar.SampleApplication.utils.Teapot;
...
public class ImageTargetRenderer extends SampleRendererBase implements SampleAppRendererControl
{
    ...
    private static final String LOGTAG = "ARNavigator";
    ...
    public static final String TAG = "Kullow";
    ...
    public void renderFrame(State state, float[] projectionMatrix)
    {
        ...
        for (TrackableResult result : trackableResultList)
        {
            ...
            if (result.isOfType(ImageTargetResult.getClassType()))
            {
                ...
                textureIndex = trackable.getName().equalsIgnoreCase("Engineering_Building") ? 0
                    : 1;
                textureIndex = trackable.getName().equalsIgnoreCase("Student_Building") ? 1
                    : textureIndex;
                textureIndex = trackable.getName().equalsIgnoreCase("IDcard") ? 0
                        : textureIndex;
    ...
    private void renderModel(float[] projectionMatrix, float[] viewMatrix, float[] modelMatrix, int textureIndex)
    {
        ...
        GLES20.glVertexAttribPointer(vertexHandle, 3, GLES20.GL_FLOAT, false, 0, mymodel.getVertices());
        GLES20.glVertexAttribPointer(textureCoordHandle, 2, GLES20.GL_FLOAT, false, 0, mymodel.getTexCoords());
        ...
        if (mActivityRef.get().isDeviceTrackingActive())
        {
            GLES20.glDrawArrays(GLES20.GL_TRIANGLES, 0, mymodel.getNumObjectVertex());
        }
        else
        {
            GLES20.glDrawElements(GLES20.GL_TRIANGLES, mymodel.getNumObjectIndex(), GLES20.GL_UNSIGNED_SHORT, mymodel.getIndices());
        }
    ...
    private void renderModel1(float[] projectionMatrix, float[] viewMatrix, float[] modelMatrix, int textureIndex)
    {
        MeshObject model;
        mymodel = new MyModel();
        float[] modelViewProjection = new float[16];
